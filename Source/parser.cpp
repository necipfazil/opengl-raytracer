...
void parser::Scene::loadFromXml(const std::string& filepath)
{
    ...
    while (element)
    {
        ...
        while (!(stream >> face.v0_id).eof())
        {
            ...
            // compute normal
            Vector3 v1 = vertex_data[face.v1_id].position - vertex_data[face.v0_id].position;
            Vector3 v2 = vertex_data[face.v2_id].position - vertex_data[face.v0_id].position;
            Vector3 normal = (v1 * v2).normalize();
            
            // add the normal to normals of each vertex
            vertex_data[face.v0_id].normal = vertex_data[face.v0_id].normal + normal;
            vertex_data[face.v1_id].normal = vertex_data[face.v1_id].normal + normal;
            vertex_data[face.v2_id].normal = vertex_data[face.v2_id].normal + normal;
    ...
    // normalize normals of each vertex
    for(int i = 0; i < (int)vertex_data.size(); i++)
    {
    	vertex_data[i].normal.normalize();
    }
